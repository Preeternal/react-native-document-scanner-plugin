...
`@preeternal/react-native-document-scanner-plugin` workspace package. It does
not use CocoaPods.

The app resolves the library from the monorepo through React Native autolinking
and Metro, without a `file:..` dependency. `react-native-image-picker` needs a
small committed package patch because the React Native 0.87 scaffolder omits
two Folly compiler flags from its generated manifest.
